...
from transformers import SamModel, SamProcessor, SamVisionConfig,SamImageProcessor
...
def forward_embeddings(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
    ...
    patch_embeds = self.projection(pixel_values.to(dtype=target_dtype))  # shape = [*, width, grid, grid]
...
class SamVisionTower(nn.Module):
    def __init__(self, vision_tower, freeze_vision=False,  input_image_size=1024, select_layer=0, select_feature='patch', delay_load=False):
        super().__init__()
    ...
    def load_model(self):
        ...
        cls_=self.vision_tower.vision_encoder.patch_embed
